...
Class SQriLiteDB {
   ...
   ; ===================================================================================================================
	Class _Table {
	 ...
	 ; ----------------------------------------------------------------------------------------------------------------
      ; METHOD NextNamed   Get next row depending on _CurrentRow; same as Next() but returns array with column names
      ; Parameters:        ByRef Row   - Variable to pass out the row array
      ; Return values:     On failure  - False, -1 for EOR (end of rows)
      ;                    On success  - True, Row contains a valid array
      ; ----------------------------------------------------------------------------------------------------------------
...
class SQLiteDB_namedtable extends SQLiteDB {
   ;----------------------------------------------------------------------------------------------------------------
   ; METHOD NextNamed   Get next row depending on _CurrentRow; same as Next() but returns array with column names
   ; Parameters:        ByRef Row   - Variable to pass out the row array
   ; Return values:     On failure  - False, -1 for EOR (end of rows)
   ...
   ;===================================================================================================================
   ; METHOD GetNamedTable  Exactly the same as GetTable, but returns named objects instead of numbered ones.
   ; Parameters:           SQL         - SQL SELECT statement
   ;                       ByRef TB    - Variable to store the result object (TB _Table)
   ;                       MaxResult   - Number of rows to return:
   ;                          0          Complete result (default)
   ...
   ;                       On failure  - False, ErrorMsg / ErrorCode contain additional information
   ; ===================================================================================================================
		GetNamedTable(SQL, ByRef TB, MaxResult := 0) {
			TB := ""
			This.ErrorMsg := ""
			This.ErrorCode := 0
			This.SQL := SQL
			...
			Loop, %GetRows% {
				...
				TB.Rows[I] := []
				Loop, %Cols% {
					TB.Rows[I][Names[A_Index]] := StrGet(NumGet(Table+0, Offset, "UPtr"), "UTF-8")
					Offset += A_PtrSize
				}
			}
			If (GetRows)
